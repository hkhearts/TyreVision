...
def analyze_tire():
    ...
    features = {
        'tread_depth_mm':       round(tread_depth, 2),
        'tread_wear_variance':  round(wear_variance, 3),
        'shoulder_wear_idx':    round(min(1.0, wear_pct * 0.6 + rng.uniform(0, 0.15)), 3),
        'sidewall_crack_density': round(crack_density, 3),
        'bulge_detected':       wear_pct > 0.9 and rng.random() < 0.3,
        'bulge_confidence':     round(max(0, wear_pct * 0.5 - 0.2 + rng.uniform(-0.1, 0.2)), 3),
        'ozone_aging_score':    round(min(1.0, (dot_info['age_months'] / 120) + rng.uniform(0, 0.15)), 3),
        'cut_puncture_count':   int(max(0, wear_pct * 3 + rng.gauss(0, 0.5))),
        'tire_age_months':      round(dot_info['age_months'], 1),
        'retread_flag':         dot_info['retread'],
        'heat_damage_idx':      round(min(1.0, wear_pct * 0.4 + rng.uniform(0, 0.2)), 3),
        'rim_corrosion_lvl':    round(rng.uniform(0, 0.3), 3),
        'valve_integrity':      round(1.0 - rng.uniform(0, 0.2), 3),
        'tire_position':        pos_key,
        'mileage_since_last':   mileage_km,
        'capture_light_quality': quality['overall'],
        'ambient_temp_c':       ambient_temp,
        'exposed_cords':        tread_depth < 1.5,
        'sidewall_crack_mm':    round(crack_density * 35, 1),
    }
